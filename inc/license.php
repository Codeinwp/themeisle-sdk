...
if ( ! class_exists( 'THEMEISLE_LICENSE' ) ) :
	...
	class THEMEISLE_LICENSE extends THEMEISLE_SDK {
		...
		public function __construct( $base ) {
			$this->product_version    = $base->product_version;
			$this->product_slug       = $base->product_slug;
			$this->product_name       = $base->product_name;
			$this->product_type       = $base->product_type;
			$this->store_url          = $base->store_url;
			$this->store_name         = $base->store_name;
			$this->paid               = $base->paid;
			$this->product_data       = $base->product_data;
			...
			if ( ! $this->paid ) {
				...
				$this->failed_checks = intval( get_option( $this->product_normalized . '_failed_checks', 0 ) );
				if ( $license_data !== '' ) {
		...
		public function license_view() {
			...
			$activate_string   = apply_filters( $this->product_normalized . '_lc_activate_string', 'Activate' );
			$deactivate_string = apply_filters( $this->product_normalized . '_lc_deactivate_string', 'Deactivate' );
			$valid_string      = apply_filters( $this->product_normalized . '_lc_valid_string', 'Valid' );
			$invalid_string    = apply_filters( $this->product_normalized . '_lc_invalid_string', 'Invalid' );
			$license_message   = apply_filters( $this->product_normalized . '_lc_license_message', 'Enter your license from %s purchase history in order to get %s updates' );

			echo '<p ><input ' . ( ( $status === 'valid' ) ? ( 'style="border:1px solid #7ad03a; "' ) : '' ) . ' type="text" id="' . $this->product_normalized . '_license" name="' . $this->product_normalized . '_license" value="' . $value . '" /><a ' . ( ( $status === 'valid' ) ? ( 'style="color:#fff;background:  #7ad03a; display: inline-block;text-decoration: none;font-size: 13px;line-height: 26px;height: 26px; margin-left:5px; padding: 0 10px 1px;  -webkit-border-radius: 3px;border-radius: 3px; ">' . $valid_string ) : ( 'style="color:#fff;background:  #dd3d36; display: inline-block;text-decoration: none;font-size: 13px;line-height: 26px;height: 26px; margin-left:5px; padding: 0 10px 1px;  -webkit-border-radius: 3px;border-radius: 3px; ">' . $invalid_string ) ) . ' </a>&nbsp;&nbsp;&nbsp;<button name="' . $this->product_normalized . '_btn_trigger" ' . ( ( $status === 'valid' ) ? ( ' class="button button-primary">' . $deactivate_string ) : ( ' class="button button-primary" value="yes" type="submit" >' . $activate_string ) ) . ' </button></p><p class="description">' . sprintf( $license_message, '<a  href="' . $this->store_url . '">' . $this->store_name . '</a> ', $this->product_type ) . '</p>';
		...
		public function get_license_status() {
			$license_data = get_option( $this->product_normalized . '_license_data', '' );
			if ( $license_data !== '' ) {
				return isset( $license_data->license ) ? $license_data->license : get_option( $this->product_normalized . '_license_status', 'not_active' );
			} else {
				return get_option( $this->product_normalized . '_license_status', 'not_active' );
			}
		...
		function show_notice() {
			...
			$status = $this->get_license_status();
			$no_activations_string = apply_filters( $this->product_normalized . '_lc_no_activations_string', 'No activations left for %s !!!. You need to
									upgrade your plan in order to use %s on more
									websites. Please ask the %s
									Staff for more details.' );
			$no_valid_string       = apply_filters( $this->product_normalized . '_lc_no_valid_string', 'In order to benefit from updates and support for %s , please add
								your license code from your  <a href="%s" target="_blank">purchase history</a> and validate it <a
									href="%s">here</a> ' );
			$expiration_string     = apply_filters( $this->product_normalized . '_lc_expiration_string', 'Your license is about to expire
									for %s. You can go to %s and renew it   ' );
			$hide_notice_string    = apply_filters( $this->product_normalized . '_lc_hide_notice_string', 'Hide Notice' );
			if ( $status != 'valid' ) {
				if ( $this->check_activation() ) {
					if ( $this->check_hide( 'activation' ) ) {
						...
						<div class="error">
							<p><strong><?php
									echo sprintf( $no_activations_string, $this->product_name, $this->product_name, '<a href="' . $this->store_url . '"
									                            target="_blank">' . $this->store_name . '</a>' );
									?></strong> | <a
									href="<?php echo add_query_arg( $this->product_normalized . "_activation", "yes" ); ?> "><?php echo $hide_notice_string; ?></a>
							</p>
						</div>
						<?php
						return false;
				...
				<?php if ( $this->check_hide( 'valid' ) ): ?>
					<div class="error">
						<p>
							<strong><?php echo sprintf( $no_valid_string, $this->product_name . ' ' . $this->product_type, $this->store_url, admin_url( "options-general.php" ) . '#' . $this->product_normalized ); ?> </strong>|
							<a
								href="<?php echo add_query_arg( $this->product_normalized . "_hide_valid", "yes" ); ?> "><?php echo $hide_notice_string; ?></a>
						</p>
					</div>
				<?php endif; ?>
				<?php
				...
				if ( $this->check_expiration() ) {
					if ( $this->check_hide( 'expiration' ) ) {
						...
						<div class="update-nag">
							<p>
								<strong><?php echo sprintf( $expiration_string, $this->product_name . ' ' . $this->product_type, '<a
										href="' . $this->renew_url() . '"
										target="_blank">' . $this->store_name . '</a>' ); ?> </strong> | <a
									href="<?php echo add_query_arg( $this->product_normalized . "_hide_expiration", "yes" ); ?> "><?php echo $hide_notice_string; ?></a>
							</p>
						</div>
						<?php
					}
		...
		private function increment_failed_checks() {
			$this->failed_checks ++;
			update_option( $this->product_normalized . '_failed_checks', $this->failed_checks );
		}
